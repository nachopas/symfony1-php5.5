...
class sfWidgetFormFilterInput extends sfWidgetForm
{
  ...
  public function render($name, $value = null, $attributes = [], $errors = [])
  {
    ...
    return strtr($this->getOption('template'), ['%input%'          => $this->renderTag('input', array_merge(['type' => 'text', 'id' => $this->generateId($name), 'name' => $name.'[text]', 'value' => $values['text']], $attributes)), '%empty_checkbox%' => $this->getOption('with_empty') ? $this->renderTag('input', ['type' => 'checkbox', 'name' => $name.'[is_empty]', 'checked' => $values['is_empty'] ? 'checked' : '']) : '', '%empty_label%'    => $this->getOption('with_empty') ? $this->renderContentTag('label', $this->translate($this->getOption('empty_label')), ['for' => $this->generateId($name.'[is_empty]')]) : '']);
  }
}
